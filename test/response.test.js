...
// Testing subject
import { coerceToArrayBuffer, Fido2AssertionResult, Fido2AttestationResult, Fido2Result } from "./helpers/lib-or-dist.js";
...
describe("Fido2AttestationResult", function () {
  ...
  it(
    "passes with 'u2f' attestation",
    async function () {
      const ret = await Fido2AttestationResult.create(h.lib.makeCredentialAttestationU2fResponse, {
        origin: "https://localhost:8443",
      ...
      const u2fTransports = ret.audit.info.get("fido-u2f-transports");
  ...
  it(
    "passes with Hypersecu u2f attestation",
    async function () {
      const ret = await Fido2AttestationResult.create(h.lib.makeCredentialAttestationHypersecuU2fResponse, {
  ...
  it(
    "passes with Sam's first u2f attestation",
    async function () {
      const samAnon1 = {
      ...
      const samAnon1Challenge = "kNWoUtrQ0O2pxKd84IaZOJ-CLJ69eevmV-o8bHcThxgoJ-lsrEZUPhWLWzw4ZJKzZeoBDdNZucIEyUm_4cur2Q";
      const ret = await Fido2AttestationResult.create(samAnon1, {
        origin: "https://webauthn.org",
  ...
  it(
    "passes with Sam's second u2f attestation",
    async function () {
      const ffNonAnon = {
        "rawId": coerceToArrayBuffer(
      ...
      const ffNonAnonChallenge = "QQTc246fjLHnnwNrminCKyJE-Ng3skW30uq4L2vqx_xNdj9ZIa4B3AGhw6f__RijOs6SbbP6m6lkLcMJG4gRYg";
      const ret = await Fido2AttestationResult.create(ffNonAnon, {
        origin: "https://webauthn.org",
  ...
  it(
    "passes with 'packed' attestation",
    async function () {
      const ret = await Fido2AttestationResult.create(h.lib.makeCredentialAttestationPackedResponse, {
        origin: "https://webauthn.org",
      ...
      // audit
      const auditInfo = ret.audit.info;
  ...
  it(
    "passes with 'tpm' attestation",
    async function () {
      const ret = await Fido2AttestationResult.create(h.lib.makeCredentialAttestationTpmResponse, {
        origin: "https://webauthn.org",
