...
const testParse = function () {
	...
	test("coerces string challenge to base64url", async () => {
		let exp = {
		...
		assert.equal(ret.get("challenge"), exp.challenge);
	});
	test("coerces Array challenge to base64url", function() {
		let exp = {
			origin: "https://webauthn.bin.coffee",
			challenge: [
		...
		assert.equal(ret.size, 2);
		assert.equal(ret.get("origin"), exp.origin);
		assert.equal(ret.get("challenge"), base64UrlChallenge);
	});
