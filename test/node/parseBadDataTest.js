...
import { coerceToArrayBuffer, tools } from "../../dist/webauthn.js";
...
describe("bad attestationObject", function () {
  ...
  it("missing fmt", async function () {
    ...
    await expect((async function () {
      await parseAttestationObject(
        coerceToArrayBuffer(tools.cbor.encode(missingFmt), "missingFmt"),
      );
    })()).to.be.rejectedWith(
      Error,
      "expected attestation CBOR to contain a 'fmt' string",
    );
  });
  it("fmt not string", async function () {
    ...
    await expect((async function () {
      await parseAttestationObject(
        coerceToArrayBuffer(tools.cbor.encode(fmtAdNumber), "fmtAdNumber"),
      );
    })()).to.be.rejectedWith(
      Error,
      "expected attestation CBOR to contain a 'fmt' string",
  ...
  it("missing attStmt", async function () {
    ...
    await expect((async function () {
      const res = await parseAttestationObject(
        coerceToArrayBuffer(
