...
class MdsEntry {
  ...
  constructor(mdsEntry, tocEntry) {
    ...
    // attachmentHint
    this.attachmentHint = this.attachmentHint instanceof Array
      ? this.attachmentHint
      : attachmentHintToArr(this.attachmentHint);
    ...
    this.attestationTypes = this.attestationTypes.map((att) =>
      typeof (att) === "string" ? att : attestationTypeToStr(att)
    );
    ...
    this.authenticationAlgorithm =
      typeof (this.authenticationAlgorithm) === "string"
        ? this.authenticationAlgorithm
    ...
    // keyProtection
    this.keyProtection = this.keyProtection instanceof Array
      ? this.keyProtection
      : keyProtToArr(this.keyProtection);
    ...
    if (this.publicKeyAlgAndEncodings) {
      this.publicKeyAlgAndEncoding = `ALG_KEY_${
        this.publicKeyAlgAndEncodings[0].toUpperCase()
      }`;
    }

    this.publicKeyAlgAndEncoding =
      typeof (this.publicKeyAlgAndEncoding) === "string"
        ? this.publicKeyAlgAndEncoding
    ...
    // tcDisplay
    this.tcDisplay = this.tcDisplay instanceof Array
      ? this.tcDisplay
      : tcDisplayToArr(this.tcDisplay);
      ...
      uvList.forEach((uv) => {
        ...
        const d = uv.map((desc) => {
          ...
          if (desc.userVerificationMethod) {
            newDesc.userVerification =
              (desc.userVerificationMethod.match(/(\w+)_internal/) ||
                ["none", "none"])[1];
          }
