import { abEqual, abToHex, b64ToJsObject, coerceToArrayBuffer, coerceToBase64Url, jsObjectToB64, tools } from "./utils.js";
...
class MdsEntry {
  ...
  constructor(mdsEntry, tocEntry) {
    ...
    function attachmentHintToArr(hint) {
      ...
      if (hint & 0xFF00) {
        throw new Error(
          "unknown attachment hint flags: " + hint & 0xFF00,
        );
      }
      return ret;
    }
    ...
    if (!Array.isArray(this.attestationTypes)) {
      throw new Error(
        "expected attestationTypes to be Array, got: " +
    ...
    this.authenticationAlgorithm = typeof (this.authenticationAlgorithm) === "string"
      ? this.authenticationAlgorithm
      : algToStr(this.authenticationAlgorithm);
    ...
    // matcherProtection
    this.matcherProtection = this.matcherProtection instanceof Array
      ? this.matcherProtection
      : matcherProtToArr(this.matcherProtection);
    ...
    this.publicKeyAlgAndEncoding = typeof (this.publicKeyAlgAndEncoding) === "string"
      ? this.publicKeyAlgAndEncoding
      : pkAlgAndEncodingToStr(this.publicKeyAlgAndEncoding);
    ...
    function tcDisplayToArr(tcd) {
      ...
      if (tcd & 0xFFE0) {
        throw new Error(
          "unknown transaction confirmation display flags: " + tcd &
            0xFFE0,
        );
      }
      return ret;
    ...
    this.userVerificationDetails = uvDetailsToSet(
    ...
    function uvDetailsToSet(uvList) {
      ...
      if (!Array.isArray(uvList)) {
        throw new Error(
          "expected userVerificationDetails to be an Array, got: " +
            uvList,
        );
      }
      uvList.forEach((uv) => {
        if (!Array.isArray(uv)) {
          throw new Error(
            "expected userVerification to be Array, got " + uv,
          );
        }
        const d = uv.map((desc) => {
          /**
          ...
          if (desc.userVerificationMethod) {
            newDesc.userVerification = (desc.userVerificationMethod.match(
              /(\w+)_internal/,
            ) ||
              ["none", "none"])[1];
          }
...
class MdsCollection {
  ...
  async addToc(tocStr, rootCert, crls) {
    ...
    if (rootCert === undefined) {
      // kid
      if (
        parsedJws.kid === "Metadata TOC Signer 3" ||
        parsedJws.key && parsedJws.key.kid === "Metadata TOC Signer 3"
      ) {
    ...
    if (this.toc.entries.some((entry) => !entry.metadataStatement)) {
      console.warn(
        "[DEPRECATION WARNING] FIDO MDS v2 will be removed in October 2022. Please update to MDS v3!",
      );
    }
  ...
  async validate() {
    ...
    for (mapEntry of this.unvalidatedEntryList) {
      ...
      tocEntryHash = coerceToArrayBuffer(
        tocEntryHash,
        "MDS TOC entry hash",
      );
      ...
      if (Array.isArray(entryId)) {
        ...
        entryId.forEach((id) => {
          this.entryList.set(
            tocEntry.metadataStatement ? id.replace(/-/g, "") : id,
            newEntry,
          );
        });
      } else {
        // UAF and FIDO2
        this.entryList.set(
          tocEntry.metadataStatement ? entryId.replace(/-/g, "") : entryId,
          newEntry,
        );
      }
    }
...
function getMdsEntryId(obj) {
  if (typeof obj !== "object") {
    throw new Error(
      "getMdsEntryId expected 'obj' to be object, got: " + obj,
    );
  }
...
function idEquals(id1, id2) {
  ...
  if (Array.isArray(id1) && Array.isArray(id2)) {
    ...
    const allSame = id1.reduce(
      (acc, val) => acc && id2.includes(val),
      true,
    );
