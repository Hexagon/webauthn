import { abEqual, abToBuf, abToInt, abToPem, appendBuffer, coerceToArrayBuffer, coerceToBase64, tools } from "../utils.js";
...
function tpmParseFn(attStmt) {
  ...
  // pubArea
  const pubArea = parsePubArea(
    coerceToArrayBuffer(attStmt.pubArea, "pubArea"),
  );
...
function parseCertInfo(certInfo) {
  ...
  if (magic !== 0xff544347) { // 0xFF + 'TCG'
    throw new Error(
      "tpm attestation: certInfo had bad magic number: " +
        magic.toString(16),
    );
  }
  ci.set("magic", magic);
  ...
  if (offset !== certInfo.byteLength) {
    throw new Error(
      "tpm attestation: left over bytes when parsing cert info",
    );
  }
...
async function tpmValidateFn() {
  ...
  const credentialPublicKeyJwk = this.authnrData.get(
  ...
  if (
    ...
    throw new Error(
      "tpm attestation: only RSA keys are currently supported",
    );
  }
  ...
  const extraDataHashBuf = tools.hashDigest(
  ...
  const pubAreaNameHashBuf = tools.hashDigest(
  ...
  try {
    ...
    const err = e;
    if (
      err.message === "Please provide issuer certificate as a parameter"
    ) {
