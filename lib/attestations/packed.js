import { abEqual, abToPem, appendBuffer, coerceToArrayBuffer, coerceToBase64, tools } from "../utils.js";
...
function packedParseFn(attStmt) {
  ...
  if (algEntry === undefined) {
    throw new Error(
      "packed attestation: unknown algorithm: " + attStmt.alg,
    );
  }
...
async function validateCerts(parsedAttCert, aaguid, _x5c, audit) {
  ...
  if (attCert.getVersion() !== 3) {
    throw new Error(
      "expected packed attestation certificate to be x.509v3",
    );
  }
  ...
  if (
    subject.get("organizational-unit-name") !== "Authenticator Attestation"
  ) {
    throw new Error(
...
async function validateSelfSignature(
  rawClientData,
  authenticatorData,
  sig,
  hashAlg,
  ...
  // verify signature
  const verify = tools.verifySignature(
    publicKeyPem,
    sig,
    appendBuffer(authenticatorData, clientDataHash),
    hashAlg,
