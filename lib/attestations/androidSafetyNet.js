import { abToStr, appendBuffer, coerceToBase64, tools } from "../utils.js";
...
async function androidSafetyNetValidateFn() {
  ...
  if (!parsedJws.payload.ctsProfileMatch) {
    throw new Error(
      "android-safetynet attestation: ctsProfileMatch: the device is not compatible",
    );
  }
  ...
  if (nonce !== parsedJws.payload.nonce) {
    throw new Error(
      "android-safetynet attestation: nonce check hash failed",
    );
  }
  ...
  if (parsedJws.payload.error) {
    throw new Error(
      "android-safetynet: " + parsedJws.payload.error + "advice: " +
        parsedJws.payload.advice,
    );
  }
  ...
  const attCert = new Certificate(
    coerceToBase64(parsedJws.header.x5c.shift(), "parsedAttCert"),
  );
  this.audit.info.set(
    "organization-name",
    attCert.getSubject().get("organization-name"),
  );
