...
async function validateAudit() {
  ...
  if (journal.size !== (clientData.size + authnrData.size)) {
    throw new Error(
      `internal audit failed: ${journal.size} fields checked; expected ${
        clientData.size + authnrData.size
      }`,
    );
  }
...
async function validateExpectations() {
  ...
  if (req.size !== exp.size - optCount) {
    throw new Error(
      `wrong number of expectations: should have ${req.size} but got ${
        exp.size - optCount
      }`,
    );
  }
      ...
      if (!Array.isArray(allowCredentials)) {
        ...
        for (const index in allowCredentials) {
          ...
          if (
            ...
            allowCredentials[index].transports != null &&
            !allowCredentials[index].transports.every((el) =>
              ["usb", "nfc", "ble", "internal"].includes(el)
            )
          ) {
            throw new Error(
              "expected transports of allowCredentials[" + index +
