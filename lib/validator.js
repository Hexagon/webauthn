  ...
  if (
    ...
    throw new TypeError(
      "expected 'id' or 'rawId' field of request to be ArrayBuffer, got rawId " +
        typeof req.rawId + " and id " + typeof req.id,
    );
  }
...
async function validateTransports() {
  ...
  for (const index in transports) {
    if (typeof transports[index] !== "string") {
      throw new Error(
        "expected transports[" + index + "] to be 'string'",
      );
    }
  }
...
async function validateId() {
  ...
  if (allowCredentials != undefined) {
    if (
      !allowCredentials.some((cred) => {
        let result = abEqual(rawId, cred.id);
        return result;
      })
    ) {
      throw new Error(
        "Credential ID does not match any value in allowCredentials",
      );
    }
  }
    ...
    if (expFlag === "UP-or-UV") {
      if (flags.has("UV")) {
        if (flags.has("UP")) {
          ...
          throw new Error(
            "expected User Presence (UP) flag to be set if User Verification (UV) is set",
          );
        }
...
async function validatePublicKey() {
  ...
  if (typeof jwk !== "object") {
    throw new Error(
      "authnrData credentialPublicKeyJwk isn't of type Object",
    );
  }
  ...
  if (typeof pem !== "string") {
    throw new Error(
      "authnrData credentialPublicKeyPem isn't of type String",
    );
  }
...
async function validateAudit() {
  ...
  if (journal.size !== (clientData.size + authnrData.size)) {
    throw new Error(
      `internal audit failed: ${journal.size} fields checked; expected ${clientData.size + authnrData.size}`,
    );
  }
...
async function validateExpectations() {
  ...
  if (exp.has("challenge")) {
    ...
    if (typeof challenge !== "string") {
      throw new Error(
        "expected challenge should be of type String, got: " +
          typeof challenge,
      );
    }
  ...
  if (exp.has("allowCredentials")) {
    ...
    if (allowCredentials != null) {
      if (!Array.isArray(allowCredentials)) {
        throw new Error(
          "expected allowCredentials to be null or array",
        );
      } else {
        for (const index in allowCredentials) {
          ...
          if (
            allowCredentials[index].id == null ||
            !(allowCredentials[index].id instanceof ArrayBuffer)
          ) {
            throw new Error(
              "expected id of allowCredentials[" + index +
                "] to be ArrayBuffer",
            );
          }
          if (
            allowCredentials[index].type == null ||
            allowCredentials[index].type !== "public-key"
          ) {
            throw new Error(
              "expected type of allowCredentials[" + index +
                "] to be string with value 'public-key'",
            );
          }
          if (
            allowCredentials[index].transports != null &&
            !Array.isArray(allowCredentials[index].transports)
          ) {
            throw new Error(
              "expected transports of allowCredentials[" + index +
                "] to be array or null",
            );
          } else if (
            allowCredentials[index].transports != null &&
            !allowCredentials[index].transports.every((el) => ["usb", "nfc", "ble", "internal"].includes(el))
          ) {
            throw new Error(
              "expected transports of allowCredentials[" + index +
                "] to be string with value 'usb', 'nfc', 'ble', 'internal' or null",
            );
          }
        }
      }
