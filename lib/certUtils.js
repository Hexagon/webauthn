import { abToStr, coerceToArrayBuffer, isPem, pemToBase64, tools } from "./utils.js";

class Certificate {
  constructor(cert) {
    // Extract base64-part from PEM
    if (isPem(cert)) {
  ...
  getSubject() {
    ...
    for (const subject of subjectItems) {
      const kv = resolveOid(
        subject.type,
        decodeValue(subject.value.valueBlock),
      );
  ...
  switch (id) {
    ...
    case "1.3.6.1.5.5.7.48.2":
      ...
      if (typeof value !== "object") {
        throw new Error(
          "expect cert-authority-issues to have Object as value",
        );
      }
...
function decodeFidoAaguid(value) {
  ...
  if (value.byteLength !== 18) {
    throw new Error(
      "AAGUID ASN.1 was wrong size. Should be 18, got " +
        value.byteLength,
    );
  }
...
function decodeAltNames(value) {
  ...
  altNames = altNames.map((name) => {
    ...
    if (name.type !== 4) {
      throw new Error(
        "expected all alternate names to be of general type",
      );
    }
...
function decodeAuthorityInfoAccess(v) {
  if (typeof v !== "object" || !Array.isArray(v.accessDescriptions)) {
    throw new Error(
      "expected authority info access descriptions to be Array",
    );
  }

  const retMap = new Map();
  v.accessDescriptions.forEach((desc) => {
    const { id, value } = resolveOid(
      desc.accessMethod,
      desc.accessLocation,
    );
    retMap.set(id, value);
...
function decodeGeneralName(type, v) {
  ...
  switch (type) {
    ...
    case 4: // directoryName
      if (!Array.isArray(v)) {
        throw new Error(
          "expected general name 'directory name' to be Array",
        );
      }
...
class CertManager {
  ...
  static async verifyCertChain(certs, roots, crls) {
    if (
      ...
      throw new Error(
        "expected 'certs' to be non-empty Array, got: " + certs,
      );
    }
    ...
    if (
      ...
      throw new Error(
        "expected 'roots' to be non-empty Array, got: " + roots,
      );
    }
    ...
    if (!Array.isArray(crls)) {
      throw new Error(
        "expected 'crls' to be undefined or Array, got: " + crls,
      );
    }
